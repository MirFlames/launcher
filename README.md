...
- [done] Перевести пользователей со старых лаунчеров (без автообновления) на новые — показывать предупреждение «версия устарела» (на backend уже есть учёт версий лаунчера у игроков).
- [done] Админ-панель на отдельном закрытом порту (желательно отдельный контейнер). Для начала: отслеживание игроков — кто на какой версии лаунчера и когда последний раз заходил в игру (в читаемом виде).
...
- [done] Создание комфортного CI/CD для минимизации ручных действий при нововведениях (попробовать github actions)
- [done] Внедрить сборку и [in_process] автоматические тесты (используя github actions, например)
- Перейти с HTTP на HTTPS
- [done] Доработать логирование: логи должны храниться в папке logs, 1 лог файл = 1 запуск лаунчера. Нейминг лог-файлов: YYYY-MM-DD-NUM.log, где NUM - порядковый номер лог-файла, если за день было несколько запусков лаунчера.
