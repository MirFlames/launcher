...
### Профили окружения (прод / dev-стенд)

Настройки → 5 тапов по версии → «Для разработчиков» → переключатель **Прод / Dev-стенд**.
Он меняет весь сетевой набор разом, чтобы не перепечатывать адреса при каждом переходе
на стенд и обратно.

В `launcher-config.json` это два поля (`config.go`):

- `env` — активный профиль (`prod` | `dev`; пусто = `prod`);
- `env_profiles` — сохранённые наборы `{apiBaseUrl, server_host, server_port, socks_proxy_*}`.

Верхнеуровневые `apiBaseUrl`/`server_host`/`server_port`/`socks_proxy_*` остаются
**эффективными** значениями — их читает весь остальной код. При сохранении настроек
`Config.ApplyProfile` копирует в них активный профиль, поэтому переключение окружения
не требует правок в downloader/jdk/modpack/launch/auth.

Пустое поле прод-профиля означает «взять дефолт сборки», так что нетронутый прод
продолжает следовать за `.env`, с которым собран бинарник, и не замерзает в конфиге
пользователя. Для dev-профиля дозаполнения нет — иначе недонастроенный стенд молча
увёл бы разработчика на прод.

Конфиги, созданные до появления профилей, раскладываются при чтении
(`Config.migrateEnvProfiles`): набор, совпадающий с дефолтами сборки, считается прод-профилем,
любой другой — dev-профилем. Эффективные поля миграция не трогает.

Файл сессии (`auth/launcher-auth.json`) один на все окружения, поэтому при смене
эффективного `apiBaseUrl` `App.SaveConfig` удаляет локальную сессию — иначе лаунчер
пошёл бы на новый бэкенд со старым `session_uuid`, а при недоступном бэкенде `verify`
молчит и протухшая сессия сошла бы за валидную.
